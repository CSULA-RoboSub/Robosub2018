...
while True:
    ...
    dice = dc.classify(frame,  interest_regions)
    window = None
    for x, y, w, h  in interest_regions:
        cv2.rectangle(clone,  (x,  y),  (x+w,  y+h),  utils.colors["blue"],  3)
    if dice != []:
        for x, y, w, h  in dice:
            cv2.rectangle(clone,  (x,  y),  (x+w,  y+h),  utils.colors["red"],  3)
            window = frame[y:y+h, x:x+w, :]
            print(detector.get_dots(window))
        '''
